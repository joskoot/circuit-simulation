    ...
    (((sub-out ...) (sub-circuit-constr sub-in ...))
     (cond
       ...
       (else
         ...
         (cons
           #`((sub-out ...)
              (sub-circuit-constr #,@(rename-hidden temps (syntax->list #'(sub-in ...)))))
...
(define-for-syntax (check-circuit-constr-args stx ins outs sub-ins sub-outs)
  ...
  (check-subset ins (append all-sub-ins outs)
    "every input must be subcircuit input or an output")
  (check-subset outs (append all-sub-outs ins)
    "every output must be a subcircuit output or an input")
  (check-subset all-sub-ins (append ins all-sub-outs)
    "Every subcircuit input must be an input or a subcircuit output")
  (check-subset all-sub-outs (append outs all-sub-ins)
